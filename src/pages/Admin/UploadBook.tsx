                          ...
                          const uploadWithProgress = (file: File, path: string, statusMsg: string): Promise<string> => {
                             return new Promise((resolve, reject) => {
                               ...
                               uploadTask.on('state_changed', 
                                 ...
                                 (error) => {
                                   console.error(`❌ Detailed upload error for ${file.name}:`, error);
                                   if (error.code === 'storage/unauthorized') {
                                      reject(new Error('عذراً، قواعد الحماية (Rules) في Firebase تمنع الرفع حالياً. يرجى التأكد من تفعيل الوصول.'));
                                   } else if (error.code === 'storage/retry-limit-exceeded') {
                                      reject(new Error('فشل الرفع بسبب ضعف الاتصال أو تجاوز عدد المحاولات.'));
                                   } else if (error.code === 'storage/canceled') {
                                      reject(new Error('تم إلغاء عملية الرفع.'));
                                   } else {
                                      reject(new Error(`خطأ في الرفع (${error.code}): ${error.message}`));
                                   }
                                 }, 
                                 async () => {
